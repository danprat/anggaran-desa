    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Laporan Kegiatan - {{ $selectedTahun->tahun }}
            </h2>
            <div class="flex space-x-2">
                <form method="POST" action="{{ route('laporan.pdf', 'kegiatan') }}" class="inline">
                    @csrf
                ...
                <form method="POST" action="{{ route('laporan.excel', 'kegiatan') }}" class="inline">
                    @csrf
                    <input type="hidden" name="tahun_id" value="{{ $selectedTahun->id }}">
                    <x-action-button 
                        type="button" 
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('laporan.show', 'kegiatan') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900">{{ $kegiatan->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900">{{ $kegiatan->where('status', 'disetujui')->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900">{{ $kegiatan->whereIn('status', ['draft', 'verifikasi'])->count() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-lg font-semibold text-gray-900">Rp {{ number_format($kegiatan->sum('pagu_anggaran'), 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Waktu Pelaksanaan</th>
                                    ...
                                    @foreach($kegiatan as $index => $item)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 text-sm text-gray-900">
                                                <div class="font-medium">{{ $item->nama_kegiatan }}</div>
                                                @if($item->keterangan)
                                                    <div class="text-gray-500 text-xs mt-1">{{ Str::limit($item->keterangan, 100) }}</div>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item->bidang }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {{ number_format($item->pagu_anggaran, 0, ',', '.') }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item->sumber_dana }}</td>
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-500 mt-2">Tidak ada data kegiatan untuk ditampilkan.</p>
                        </div>
                    @endif
                </div>
            </div>
